#include "LIFModel.hpp"
#include "../../../../include/DebugConfig.hpp"

namespace brainll {

void LIFModel::updateNeuron(std::shared_ptr<AdvancedNeuron> neuron, double dt, double input_current) {
    // Verificar que la neurona existe y es del tipo correcto
    if (!neuron || neuron->getModel() != NeuronModel::LIF) {
        DEBUG_LOG("LIFModel: Error al actualizar neurona - Neurona no válida o tipo incorrecto");
        return;
    }
    ...
    membrane_potential += (input_current - leak_current) * dt;
...
double LIFModel::calculateLeakCurrent(std::shared_ptr<AdvancedNeuron> neuron, double dt) {
...
bool LIFModel::shouldFire(std::shared_ptr<AdvancedNeuron> neuron) {
    // Obtener los parámetros necesarios
    double membrane_potential = neuron->getPotential();
    double threshold = neuron->getThreshold();
...
void LIFModel::resetAfterFiring(std::shared_ptr<AdvancedNeuron> neuron, double current_time) {
    // Obtener los parámetros necesarios
